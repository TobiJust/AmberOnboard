...
uint8_t ProcPayload::push(shared_ptr<DataField> output) {
    ...
    case DATA_ACQUIRED:
    {
        ...
        status = packAcquiredData(outBuffer,
                dynamic_pointer_cast<AcquiredData_Out>(output),
...
uint8_t ProcPayload::pull_new(shared_ptr<DataField> input) {
...
uint8_t ProcPayload::unpackAcquiredData(shared_ptr<AcquiredData_In> &data,
        shared_ptr<vector<uint8_t>> &packet,
        uint8_t *&begin,
        uint8_t *&end) {
